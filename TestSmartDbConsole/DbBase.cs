   ...
   public class DbBase<Entity>
    {
        ...
        protected SqlDbFactory _dbFactory;
        protected string _dbOperator;
        ...
        {
            //根据实体主键值查询数据
            var result = _db.Delete<Entity>(3);

            //根据过滤SQL、object参数查询数据列表1
            result = _db.Delete<Entity>("UserId = 1", null);
            result = _db.Delete<Entity>(string.Format("UserId={0}UserId", _dbOperator), new { UserId = 1 });
        }
        ...
        public void Update()
        {
            ...
            var data = _db.Query<Entity>(4);
            //data.UserName = "joyet22";
            var result = _db.Update<Entity>(data);
            result = _db.Update<Entity>(new { UserName = "joyet222" }, 2);
        }
        ...
        {
            //根据实体主键参数值查询数据
            var data = _db.Query<Entity>(3);

            //根据查询字段、过滤SQL、object参数查询数据列表
            var dataList1 = _db.Query<Entity>("UserId,UserName", "UserId=3", null);
            var dataList2 = _db.Query<Entity>("UserId,UserName", string.Format("UserId={0}UserId", _dbOperator), new { UserId = 3 });

            //根据查询参数化SQL、object参数查询数据列表
            var dataList3 = _db.Query<Entity>("select * from UserInfo where UserId=3", null);
            var dataList4 = _db.Query<Entity>(string.Format("select * from UserInfo where UserId={0}UserId", _dbOperator), new { UserId = 3 });

            //分页查询列表
            var pageDataList1 = _db.QueryPageList<Entity>("UserId,UserName", "UserId", "asc", 10, 1, string.Format("UserId>{0}UserId", _dbOperator), new { UserId = 3 });
            var pageDataList2 = _db.QueryPageList<Entity>("*", "UserId", "asc", 10, 2, "UserId>2", null);
        }
        ...
        public void OrtherQuery()
        {
            ...
            {
                dataList = _db.DataReaderToEntityList<Entity>(dataReader);
            }
